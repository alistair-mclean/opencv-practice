...
vector_dim = 300
root_path = "C:\\Users\\alist\\Documents\\Practice\\Python\\neuralNetworks\\tensorflow"
...
def gensim_demo():
	...
	logging.basisConfig(format='%(asctime)s : %(levelname)s : %(message)s', level=logging.INFO)
...
def create_embedding_matrix(model):
	...
	for i in range(len(model.wv.vocab)):
		embedding_vector = mdoel.wv[model.wv.index2word[i]]
...
def tf_model(embedding_matrix, wv):
	...
	with tf.Session() as sess:
		...
		sim = similarity.eval()
		# run through each VALID example finding the closest words 
...
def keras_model(embedding_matrix, wv):
	...
	# input words - in this case we do sample by sample evaluation of  the similarityh
